...
def scanCallback(n): # Pass 1 (next Scan time) or -1 (prev Scan time)
	global scanMode
	setScanMode((scanMode + n) % len(scanData))

def photoCallback(n): # Pass 1 (next Photo time) or -1 (prev Photo time)
	global photoMode
	setPhotoMode((photoMode + n) % len(photoData))
...
def shotCallback(n): # Pass 1 (next mode) or -1 (prev mode)
	global shotMode
	setShotMode((shotMode + n) % len(shotData))
...
cacusensor		= serial.Serial('/dev/ttyUSB0', 9600) # Connection to sensor array
scanMode		=  0      # Time between sensor scans
photoMode		=  0      # Scans between photographs
shotMode		=  0      # Data capture mode (eg - snapshot, timelapse...)
logIdx			=  -1	  # Log index for saving (-1 = none set yet)
...
pathData = [
  '/home/pi/usbdrv/Photos',     # Path for storeMode = 0 (Photos folder)
  '/boot/DCIM/CANON999', 		# Path for storeMode = 1 (Boot partition)
  '/home/pi/usbdrv/Photos']     # Path for storeMode = 2 (Dropbox)
  
shotData = [					# Data capture modes
  'snapshot', 'polyspec', 'timelapse', 'security']
  
scanData = [ 					# Time between each scan in automated modes [Time value, indicator X position]
 [5, 23], [10, 60], [20, 97], [30, 137],
 [60, 174], [90, 212], [120, 250], [300, 287]
 ]
 
photoData = [ 					# Number of scans between each photo in automated modes [Iteration value, indicator X position]
 [0, 23], [1, 60], [2, 97], [3, 137],
 [6, 174], [10, 212], [30, 250], [100, 287]
 ]
...
buttons = [
  ...
  [Button((  0,188,320, 52), bg='done'   , cb=doneCallback),
   ...
   # Screen mode 9 is scan frequency
  [Button((  0,188,320, 52), bg='done', cb=doneCallback),
   Button((  0,  0, 80, 52), bg='prev', cb=settingCallback, value=-1),
   Button((240,  0, 80, 52), bg='next', cb=settingCallback, value= 1),
   Button((  0, 70, 80, 52), bg='prev', cb=scanCallback   , value=-1),
   Button((240, 70, 80, 52), bg='next', cb=scanCallback   , value= 1),
   Button((  0, 79,320, 33), bg='freq-5'),
   Button((  9,134,302, 26), bg='scan-bar'),
   Button(( 17,167, 21, 19), bg='iso-arrow'),
   Button((  0, 10,320, 29), bg='scans')],
   
   # Screen mode 10 is photo frequency
  [Button((  0,188,320, 52), bg='done', cb=doneCallback),
   Button((  0,  0, 80, 52), bg='prev', cb=settingCallback, value=-1),
   Button((240,  0, 80, 52), bg='next', cb=settingCallback, value= 1),
   Button((  0, 70, 80, 52), bg='prev', cb=photoCallback  , value=-1),
   Button((240, 70, 80, 52), bg='next', cb=photoCallback  , value= 1),
   Button((  0, 79,320, 33), bg='freq-0'),
   Button((  9,134,302, 26), bg='photo-bar'),
   Button(( 17,167, 21, 19), bg='iso-arrow'),
   Button((  0, 10,320, 29), bg='photos')],

# Screen mode 11 is mode
  [Button((  0,188,320, 52), bg='done', cb=doneCallback),
   Button((  0,  0, 80, 52), bg='prev', cb=settingCallback, value=-1),
   Button((240,  0, 80, 52), bg='next', cb=settingCallback, value= 1),
   Button((  0, 70, 80, 52), bg='prev', cb=shotCallback     , value=-1),
   Button((240, 70, 80, 52), bg='next', cb=shotCallback     , value= 1),
   Button((  0, 67,320, 91), bg='shot-snapshot'),
   Button((  0, 11,320, 29), bg='mode')],
...
def setShotMode(n):
	global shotMode
	shotMode = n
	buttons[11][5].setBg('shot-' + shotData[shotMode])
...
def setScanMode(n):
	global scanMode
	scanMode    = n
	buttons[9][5].setBg('freq-' + str(scanData[scanMode][0]))
	buttons[9][7].rect = ((scanData[scanMode][1] - 10,) +
	  buttons[9][7].rect[1:])
	  
def setPhotoMode(n):
	global photoMode
	photoMode    = n
	buttons[10][5].setBg('freq-' + str(photoData[photoMode][0]))
	buttons[10][7].rect = ((photoData[photoMode][1] - 10,) +
	  buttons[10][7].rect[1:])
...
def loadSettings():
	try:
	  ...
	  if 'shot' in d: shotCallback(d['shot'])
	  if 'scan' in d: scanCallback(d['scan'])
	  if 'photo' in d: photoCallback(d['photo'])
	except:
	  pass
...
def takePicture():	# Check shot mode and run the necessary subroutine
	global shotMode
...
def takeSnapshot():	# This handles snapshot mode
	...
	cacufile = open("log-snapshots.csv", 'a')	# open log file for sensor readings, append mode
	cacusensor.write('1')						# tell sensor array to start a scan
	...
	cacuscan = cacusensor.readline()		# read scan data from sensor array
	cacufile.write(filename+","+cacuscan)	# write scan data to log file
	cacufile.close()						# close log file
	busy = False
	t.join()
	...
	if scaled:
	  ...
	  caculist = cacuscan.split(",")
	  printText(str(caculist[1:9]), "Arial", 12, 10, 10, (255, 255, 255))	# overlay sensor data on screen
	  printText(str(caculist[10:14]), "Arial", 12, 10, 30, (255, 255, 255))	# overlay sensor data on screen
	  printText(str(caculist[14:18]), "Arial", 12, 10, 50, (255, 255, 255))	# overlay sensor data on screen
	  printText(str(caculist[18:25]), "Arial", 12, 10, 70, (255, 255, 255))	# overlay sensor data on screen
	  printText(str(caculist[25:33]), "Arial", 12, 10, 90, (255, 255, 255))	# overlay sensor data on screen
	  pygame.display.update()
	  while True:
		if (GPIO.input(22) == 0): break
		time.sleep(0.3)
	  loadIdx = saveIdx

def takePolyspec():	# This handles polyspec mode
	global busy, gid, loadIdx, saveIdx, scaled, sizeMode, storeMode, storeModePrior, uid
	
	caculoop = 0	# counter for colours used
	...
	try:
	  ...
	  while (caculoop < 4): 						# polyspec mode takes multiple photos
		  if (caculoop == 1):
			filename = pathData[storeMode] + '/IMG_' + '%04d' % saveIdx + 'r.JPG'	# red photo
		  elif (caculoop == 2):
			filename = pathData[storeMode] + '/IMG_' + '%04d' % saveIdx + 'g.JPG'	# green photo
		  elif (caculoop == 3):
			filename = pathData[storeMode] + '/IMG_' + '%04d' % saveIdx + 'b.JPG'	# blue photo
		  cacufile = open("log-snapshots.csv", 'a')	# open log file for sensor readings, append mode
		  cacusensor.write('2')						# tell sensor array to start a scan, log mode
		  camera.capture(filename, use_video_port=False, format='jpeg',
			thumbnail=None)
		  # Set image file ownership to pi user, mode to 644
		  # os.chown(filename, uid, gid) # Not working, why?
...
def takeTimelapse():	# This handles the automatic shot modes (timelapse, security)

	global scanMode, photoMode, gid, sizeMode, uid
	...
	while True:
		caculogname = '/home/pi/usbdrv/logs/log' + '%04d' % caculognum + '.csv'
		if not os.path.isfile(caculogname): break
		caculognum += 1
		if caculognum > 9999: caculognum = 0	
	os.system("echo '0' > /sys/class/gpio/gpio252/value")		# turn backlight off

	while (caculoop < 1):										# timelapse loop
		if (time.time() > cacudelay):							# if it's time to scan
			cacudelay = cacudelay + (scanData[scanMode][0]) 	# reset timer for scanning
			cacufile = open(caculogname, 'a')					# open log file	
			cacusensor.write('2')								# request log mode scan from arduino
			cacufile.write(str(GPIO.input(21))+","+str(time.time() - cacustart)+",")	# write time of scan to file
			if (cacuitphoto < 1 and photoMode > 0) or (shotMode == 3 and GPIO.input(21) > 0):	# if it's time to take a photo
				cacuframename = '/home/pi/usbdrv/logs/log' + '%04d' % caculognum + 'frame' + '%04d' % cacuframenum + '.jpg'
				camera.resolution = (1440, 1080)
				camera.crop       = (0.0, 0.0, 1.0, 1.0)
				try:
				  camera.capture(cacuframename, use_video_port=False, format='jpeg',
					thumbnail=None)
				  # Set image file ownership to pi user, mode to 644
				  os.chmod(cacuframename,
					stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP | stat.S_IROTH)
				...
				cacuitphoto = (photoData[photoMode][0]) - 1		# number of iterations -1 until next photo
			elif (photoMode > 0):
				cacuitphoto = cacuitphoto - 1				# ...advance photo timer
				time.sleep(1)								# give arduino time to respond if no photo was taken
			else:
				time.sleep(1)								# give arduino time to respond if no photo was taken
			cacuscan = cacusensor.readline()					# read scan data
			cacufile.write(str(time.time() - cacustart)+","+str(cacuframenum)+","+cacuscan)	# write data to file	
			cacufile.close()									# close file
		if ((shotMode == 3) and (GPIO.input(21) > 0)):		# if motion is detected in security mode...
			cacudelay = time.time()							# ...start recording immediately
		time.sleep(0.5)	
		if (GPIO.input(18) == 0):
			os.system("echo '0' > /sys/class/gpio/gpio252/value")	# turn backlight off
		if (GPIO.input(27) == 0):
...
os.system("echo 252 > /sys/class/gpio/export")					# open backlight port
os.system("echo 'out' > /sys/class/gpio/gpio252/direction")		# set backlight port to outgoing
os.system("echo '1' > /sys/class/gpio/gpio252/value")			# turn backlight on
